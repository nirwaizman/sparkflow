/**
 * Google adapter using the Vercel AI SDK (`ai` + `@ai-sdk/google`).
...
function client() {
  const apiKey = optionalEnv("GOOGLE_GENERATIVE_AI_API_KEY");
  if (!apiKey) {
    throw new MissingApiKeyError(PROVIDER, "GOOGLE_GENERATIVE_AI_API_KEY");
  }
