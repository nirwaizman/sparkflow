...
const menuItemBase =
  ...
  "focus:bg-[hsl(var(--muted))] focus:text-[hsl(var(--fg))] " +
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.SubContent
    ref={ref}
    className={cn(
      "z-50 min-w-[8rem] overflow-hidden rounded-md border border-[hsl(var(--border))]",
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(menuItemBase, inset && "ps-8", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <span className="absolute start-2 flex h-3.5 w-3.5 items-center justify-center">
      <DropdownMenuPrimitive.ItemIndicator>
        <span className="h-2 w-2 rounded-full bg-[hsl(var(--fg))]" />
      </DropdownMenuPrimitive.ItemIndicator>
    </span>
    {children}
  </DropdownMenuPrimitive.RadioItem>
...
export function DropdownMenuShortcut({
  ...
  return (
    <span
      className={cn(
        "ms-auto text-xs tracking-widest text-[hsl(var(--muted-foreground))]",
        className,
      )}
      {...props}
    />
