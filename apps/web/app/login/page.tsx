 ...
 *
 * Submits directly to Supabase from the browser. Supabase emails a
 * magic link that lands on `/auth/callback?code=...`, where we
 * exchange the code for a session and create a personal org on first
 * sign-in.
 *
 * We read the `next` query param and forward it through the callback
 * so users land back on the protected page they originally requested.
 */
    ...
    <main className="flex min-h-dvh items-center justify-center p-6">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            קבל קישור קסם למייל או התחבר עם חשבון Google.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          {status === "sent" ? (
            <Alert variant="success">
              ...
              <AlertDescription>
                שלחנו קישור התחברות לכתובת <strong>{email}</strong>. ייתכן שייקח
                דקה עד שהמייל יגיע.
              </AlertDescription>
            </Alert>
          ) : (
        ...
        <CardFooter className="text-xs text-[hsl(var(--muted-foreground))]">
          עדיין אין לך חשבון? הרשמה והתחברות זהות — פשוט הזן מייל.
        </CardFooter>
      </Card>
    </main>
  );
